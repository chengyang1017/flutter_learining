...
class FlutterUiParser {
  ...
  UiValue _value(ParserCursor c) {
    ...
    if (c.match(TokenType.boolean)) {
      return BooleanUiValue(c.tokens[c.index - 1].lexeme == 'true');
    }
    if (c.match(TokenType.nullValue)) return const NullUiValue();
    ...
    final first = c.expect(TokenType.identifier, '此处需要值');
    final name = StringBuffer(first.lexeme);
    while (c.match(TokenType.dot)) {
      name.write('.${c.expect(TokenType.identifier, '点号后需要标识符').lexeme}');
    }
    if (!c.match(TokenType.leftParen)) {
      return IdentifierUiValue(name.toString());
    }
    final positional = <UiValue>[], named = <String, UiValue>{};
